...
INSERT INTO User (Username, Password, PinCode, TotalCostPrescriptions,MonthlyCost, FinancialReport)
VALUES
('throwerslug','password1', '1111', '100.00', '33.2', NULL),
...
CREATE TABLE IF NOT EXISTS Insurance (
    ...
    #FOREIGN KEY (`CodePin`) REFERENCES `MrPharma`.`User` (`PinCode`)
    #INDEX `PinCode_Index` (`CodePin`)
);

ALTER TABLE Insurance
add INDEX PinCode_Index (CodePin),
add FOREIGN KEY (CodePin) REFERENCES MrPharma.User (PinCode);

ALTER TABLE Insurance
MODIFY COLUMN PhoneNumber VARCHAR(45);
...
CREATE TABLE IF NOT EXISTS MrPharma.Prescription (
  ...
  -- FOREIGN KEY (`code_pin`) REFERENCES `MrPharma`.`User` (`PinCode`),
--   INDEX `pinCode_idx` (`code_pin`),
--   FOREIGN KEY (`BrandID`) REFERENCES `MrPharma`.`PrescriptionBrand` (`BrandID`),
--   INDEX `PrescriptionBrand_idx` (`BrandID`)
);
